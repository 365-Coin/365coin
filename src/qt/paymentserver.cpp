...
const QString BITCOIN_IPC_PREFIX("365coin:");
...
{
    // Install global event filter to catch QFileOpenEvents on the mac (sent when you click 365coin: links)
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start 365coin: click-to-pay handler");
    else
...
{
    // clicking on 365coin: URLs creates FileOpen events on the Mac:
