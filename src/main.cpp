...
uint256 hashGenesisBlock("000000edc0a48a245f6370d2257276977e273b2e33fe27f2614a1ca0bd693762");
static const unsigned int timeGenesisBlock = 122834834;
static const unsigned int nNonceGenesisBlock = 52509231;
...
const string strMessageMagic = "365Coin Signed Message:\n";
                              ...
                              enum GetMinFee_mode mode) const
{
    ...
    if (fAllowFree)
    {
        if (nBlockSize == 1)
        {
            ...
            // (about 1500 365 if made of 50 365 inputs)
            if (nBytes < 100)
                nMinFee = 0;
        }
        else
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("832bcc92fbc85317b745a9e9b1e928e6a98132dcfde1966c4760990008d718b6");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis block
        const char* pszTimestamp = "how would that be any different than the ones that already exsist?";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        if (fTestNet)
        {
            block.nTime    = 122834834;
            block.nNonce   = 31449673;
        }
        ...
        assert(block.hashMerkleRoot == uint256("2d4b6242fae5db2e1b7ac58807e0d458e42cd8fb5cf6a2021a7343e626fdcb34"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("365CoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("365CoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("365CoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("365coin-miner");
    ...
    try { while(true) {
        ...
        printf("Running 365CoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//            unsigned int nNonceFound;
    ...
    {
        printf("365CoinMiner terminated\n");
        throw;
    }
}
